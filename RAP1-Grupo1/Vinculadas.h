...
void localizarVin(Lista *l, char *idBuscado, int *rLocalizar, int typeEstructura, int *cantConsultas){
    ...
    *rLocalizar = 0;
    ...
    if (l->acc != NULL){
        ...
        //typeEstructura == 2 significa que está usando LVO
            while ((l->cur != NULL) && strcmpi((l->cur->vipd.codigo), idBuscado)<0){
                l->pre = l->cur;
                l->cur = l->cur->next;
                (*cantConsultas)++;
        ...
        *rAlta = 0; //Como se encontró el elemento entonces el alta falla
        ...
        if (n != NULL){
            if (typeEstructura == 1){
                ...
                n->vipd = art;
                //typeEstructura == 2 significa que está usando LVO
                if (l->acc==NULL){ // alta en estructura vacía
                    l->acc = n;
                    n->next = NULL;
                    sumAux+= 1;
                    *rAlta = 1; // exito en el alta
                ...
                } else if (l->pre == l->cur) { //alta al principio
                    l->acc = n;
                    n->next = l->pre;
                    sumAux += 1;
                    *rAlta = 1; //exito en el alta
...
void bajaVin(Lista *l, int *rBaja, int typeEstructura, Articulo art, int lecOp, float *costoMax, float *sumCostos, int *cantExitos){
    ...
    *rBaja = 0;
    ...
    if(rLocalizar == 1){
        ...
        if (eleccion == 1){
            if(l->acc->next==NULL){ //eliminamos el único
                temp = l->cur;
                l->acc = NULL;
                (*l).cargados = 0;
                sumAux+= 0.5;
            ...
            } else if (l->cur == l->pre){ //eliminamos la 1ra
                temp = l->acc;
                l->acc = l->acc->next;
                sumAux+= 0.5;
                free(temp);
